...
class GraphTranslatorModule(LightningModule):
    def __init__(self, model_configs):
        ...
        """
        print('n_nodes is , n_len is , c_len is , edge imp is',
            self.n_nodes, self.n_len, self.c_len, self.edge_importance
        )
        """
        ...
        self.context_loss = lambda context_list: sum([torch.nn.CosineEmbeddingLoss()(con, context_list[0], torch.Tensor([1]).to('cpu')) for con in context_list]) #change to cuda todo
    ...
    def step(self, batch):
        ...
        """
        print('Shape of nodes is',nodes.shape)
        batch_size, num_nodes, node_feature_len = nodes.size()
        print('batch size is {}, num nodes is {}, node feature len is {}'.format(batch_size,num_nodes,node_feature_len))
        print('Shape of edges is',edges.shape)
        print('Shape of y edges is',y_edges.shape)
        print('Shape of y nodes is',y_nodes.shape)
        print('Shape of dynamic edges is',dyn_edges.shape)
        print('Shape of context time is ',time_context.shape)
        """
    ...
    def training_step(self, batch, batch_idx):
        loss, details, _ = self.step(batch)
        
        """
        node_ids_from_classes = {0:' kitchen',1:' fridge',2:' counter',3:' cabinet',4:' milk',5:' cereal', 6:'coffee',
                             7:' keys', 8:' cup', 9:' bowl', 10:' rack'}
        print('\n Input nodes ',batch['nodes'].tolist())
        print('\n y nodes ',batch['y_nodes'].tolist())
        print('\n Dynamic mask',batch['dynamic_edges_mask'].tolist())
        print('\n Context time is',batch['context_time'].tolist())
        print('\n Input edges ',batch['edges'].tolist())
        print('\n y edges ',batch['y_edges'].tolist())
        print('\n gt location of y_edges is ',details['gt']['location'].tolist()) #self.inference_location(y_edges)
        #with open('/workspace/SpatioTemporalObjectTracking/data/HOMER/household0/processed/common_data.json', 'r') as f:
        #    classes = json.load(f)
        #for val in range(len(details['gt']['location'].squeeze())):            
            #print('gt Object {} is on {}'.format(classes['node_classes'][val], classes['node_classes'][details['gt']['location'][0][val]])) #for homer

        for val in range(len(details['gt']['location'].squeeze())):    
            print('gt Object {} is in {}'.format(node_ids_from_classes[val], node_ids_from_classes[details['gt']['location'][0][val].item()]))
        predicted_class = details['output_probs']['location'].argmax(-1).tolist()
        print('\n predicted class', predicted_class)        
        for val in range(len(predicted_class[0])):    
            print('Prediction Object {} is on {}'.format(node_ids_from_classes[val], node_ids_from_classes[predicted_class[0][val]]))

        print('\n log  op is ',torch.log( details['output_probs']['location']) ) #todo should it be log only as we do softmax before

        """
        accuracy = int((details['output_probs']['location'].argmax(-1) == details['gt']['location']).sum()) / self.n_nodes
