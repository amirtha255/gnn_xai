...
def data_loader_conversion(x,edge_index,y_batch, time_batch, context_time_batch, dynamic_edges_mask, y_index):
    data_list=[]
    ...
    data_list.append({'edges':edges.requires_grad_(True),
                    ...
                    'dynamic_edges_mask':dynamic_edges_mask.squeeze().requires_grad_(True),'context_time':context_time.requires_grad_(True)})
    new_data_loader = DataLoader( data_list,batch_size=1,  )
    return new_data_loader

class GnnConverter(torch.nn.Module):
    def __init__(self, model_configs, y_batch,  time_batch, context_time_batch, dynamic_edges_mask, **kwargs):
        super().__init__()
        checkpoint = kwargs.get('check_point','')
        data_type =  kwargs.get('data_type','')       
        self.dynamic_edges_mask = dynamic_edges_mask
        self.y_batch = y_batch  
        self.time_batch = time_batch
        self.context_time_batch = context_time_batch
        if data_type == 'homer':
    ...
    def forward(self, x, edge_index, **kwargs):
        ...
        if not y_index:
            y_index = 0
        new_train_data_loader = data_loader_conversion(x, edge_index, self.y_batch, self.time_batch, self.context_time_batch, self.dynamic_edges_mask, y_index)
